...
public class ArticoloDAO {
	...
	public ArrayList<Articolo> findArticoliAsta(int idAsta) throws SQLException{
		...
		PreparedStatement preparedStatement = null; //permette di eseguire la query nel DB
		ResultSet result = null;
		try{ 
			preparedStatement = connection.prepareStatement(query); //COMPILO
			preparedStatement.setInt(1,idAsta); //setto i parametri
			result = preparedStatement.executeQuery(); //ESEGUO
			if(!result.isBeforeFirst()) {  //non ho trovato articoli 
				return null;
			} else {
				while(result.next()) {
	...
	//Trovo tutti gli articoli disponibili (NON appartenenti ad un'asta)
	public ArrayList<Articolo> findArticoliDisponibili(int idUser) throws SQLException{
		...
		ResultSet result = null;
		try{
			preparedStatement = connection.prepareStatement(query); //compila
			preparedStatement.setInt(1,idUser);
			result = preparedStatement.executeQuery(); //esegue
	...
	//metodo usato per aggiungere articoli in un'asta (nella tabella ARTICOLOASTA viene creata una riga con idAsta e codice)
